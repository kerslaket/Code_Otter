...
var shots = [];
var aliens = [];
var alienTexts = ["for","whiledo","if","while","truth","dowhile"];//sets text 
var alienCor = [true,false,false,true,false,true];//sets bools
var points = 0;
var won = false;
...
function background(){//background
    ...
    ctx.fillStyle="green";
    ctx.font = "25px Arial";
    ctx.fillText("Level 2 -Shoot All The Different Loops", 100 , 50);
}
...
function gameover(){//end game screen
    ...
    firebase.database().ref('Highscores2/Highscore 1/Username').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 200 , 200);
    });
    firebase.database().ref('Highscores2/Highscore 1/Score').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 500 , 200);
    });
    
    firebase.database().ref('Highscores2/Highscore 2/Username').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 200 , 250);
    });
    firebase.database().ref('Highscores2/Highscore 2/Score').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 500 , 250);
    });

    firebase.database().ref('Highscores2/Highscore 3/Username').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 200 , 300);
    });
    firebase.database().ref('Highscores2/Highscore 3/Score').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 500 , 300);
    });

    firebase.database().ref('Highscores2/Highscore 4/Username').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 200 , 350);
    });
    firebase.database().ref('Highscores2/Highscore 4/Score').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 500 , 350);
    });

    firebase.database().ref('Highscores2/Highscore 5/Username').on('value', function(snapshot){
        ctx.fillText(snapshot.val(), 200 , 400);
    });
    firebase.database().ref('Highscores2/Highscore 5/Score').on('value', function(snapshot){
            ...
            firebase.database().ref('Users/' + firebaseUser.uid + '/username').on('value', function(snapshot){
                ...
                firebase.database().ref('Highscores2/Highscore 1/Score').on('value', function(snapshot){
                    if (points > snapshot.val() && newhighscore == false){
                        firebase.database().ref('Highscores2/Highscore 1').set({
                ...
                firebase.database().ref('Highscores2/Highscore 2/Score').on('value', function(snapshot){
                    if (points > snapshot.val() && newhighscore == false){
                        firebase.database().ref('Highscores2/Highscore 2').set({
                            Username: usernamevar,
                            Score: points,
                        })
                    newhighscore = true;
                ...
                firebase.database().ref('Highscores2/Highscore 3/Score').on('value', function(snapshot){
                    if (points > snapshot.val() && newhighscore == false){
                        firebase.database().ref('Highscores2/Highscore 3').set({
                            Username: usernamevar,
                            Score: points,
                        })
                    newhighscore = true;
                ...
                firebase.database().ref('Highscores2/Highscore 4/Score').on('value', function(snapshot){
                    if (points > snapshot.val() && newhighscore == false){
                        firebase.database().ref('Highscores2/Highscore 4').set({
                            Username: usernamevar,
                            Score: points,
                        })
                    newhighscore = true;
                ...
                firebase.database().ref('Highscores2/Highscore 5/Score').on('value', function(snapshot){
                    if (points > snapshot.val() && newhighscore == false){
                        firebase.database().ref('Highscores2/Highscore 5').set({
                            Username: usernamevar,
                            Score: points,
                        })
                    newhighscore = true;
